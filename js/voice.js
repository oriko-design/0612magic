...
class SpellListener {
  ...
  _isSpell(rawText) {
    ...
    // フレーズ全体ではなく「特徴的なかけら」がひとつでも聞こえたら発動する。
    // 普通の会話にはまず出てこない音の並びなので、ゆるくても誤発動しにくい。
    const chunks = [
      /[ビピ][ビピ](ディ|デ|ジ|ティ|チ)/, // ビビディ・ビビデ・ピピディ…
      /[バパ][ビピ](ディ|デ|リ|ジ|ティ|チ)/, // バビディ・バビデ・パピディ…
      /[ビピ][ビピ]リ[バパ]/, // ビビリバ(聞き取り揺れ)
      /[バパ][ビピ][ブプ]/, // バビブ(後半だけ聞こえた時)
    ];
    if (chunks.some((re) => re.test(text))) {
      return true;
    }
    // 英語・ローマ字認識対策(bibbidi / bobbidi の断片でも OK)
    const en = rawText.toLowerCase().replace(/[^a-z]/g, "");
    return /b[iy]b+[iey]?d[iy]/.test(en) || /b[oa]b+[iey]?d[iy]/.test(en);
  }
}
